...
import org.teatrove.barista.http.ServletPool;
import org.teatrove.barista.http.BaristaRequestDispatcher;
import org.teatrove.barista.http.TQServlet;
import org.teatrove.barista.http.SortableFilter;
import org.teatrove.barista.http.AbortServlet;
import org.teatrove.barista.http.FilterConfigImpl;


/**
 * @author Tammy Wang
 */
public class UdpServletDispatcher
    implements ServletContext, UdpHandlerStage
{
	private static String DefaultNA = "na";
     ...
     * </ul>
     * <p>
     * UdpServletDispatcher must be {@link #init(Config) initialized} before
     ...
     */
    public UdpServletDispatcher(Map servletConfigs,
                                 Map filterConfigs) {
    ...
    public void init(UdpHandlerStage.Config config) {
        if (config == null) {
            destroy();
        }
        else {
    ...
    public UdpHandlerStage.Config getConfig() {
        return mConfig;
    }

    public boolean handle(UdpServerConnection con, 
                          UdpHandlerStage.Chain chain)
    {
        UdpServletRequestImpl request = new UdpServletRequestImpl(con);
        UdpServletResponseImpl response =
            new UdpServletResponseImpl(con, mCharsetAliases);
	...
	public void sendError(ServletResponse response)
	{
		try
		{
			ServletOutputStream sos = response.getOutputStream();		    
			sos.write(DefaultNA.getBytes());
			sos.flush();
		}catch(IOException ie)
		{
			mLog.error("UdpServletDispatcher::IOException");
			ie.printStackTrace();
		}
	}
    ...
    private void sendError(ServletRequest req, ServletResponse res,
                           int sc)
    {
    ...
    private void destroy() {
        ...
        for (int i=0; i<pools.length; i++) {
            try {
				...
				mLog.debug("dispather exception destroy");
                mLog.debug(e);
            }
        }
    ...
    public interface Config extends UdpHandlerStage.Config {
        public String getServerInfo();
    ...
    public static class DefaultConfig extends ConfigWrapper implements Config {
        ...
        public DefaultConfig(UdpHandlerStage.Config config) {
            super(config);
        ...
        implements FilterChain, Transaction
    {
        ...
        private UdpServletRequestImpl mRequest;
        private UdpServletResponseImpl mResponse;
        private Servlet mServlet;

        public ServletFilterChainTransaction(Filter[] filters,
                                             ServletPool pool,
                                             UdpServletRequestImpl request,
                                             UdpServletResponseImpl response)
        {
            mFilters = filters;
            mPool = pool;
        ...
        implements BaristaRequestDispatcher, FilterChain
    {
            ...
            if (request instanceof ServletRequest) {
                ...
                if (mURI == null) {
                    ...
                    query = mergeQueryStrings
                        (((UdpServletRequestImpl)request).getQueryString(),
                         mQuery);
                }
                
                // Wrap this request to hide orginal destination info.
                request = new ForwardedRequestWrapper
                    ((UdpServletRequestImpl)request, mURI, mContextPath,
        ...
        public void include(ServletRequest request, ServletResponse response)
            throws ServletException, IOException
        {
            if (request instanceof UdpServletRequestImpl &&
                response instanceof UdpServletResponseImpl) {
                ...
                if (response instanceof UdpServletResponseImpl) {
                    ((UdpServletResponseImpl)response).resetOutputType();
                }
    ...
    private class RequestWrapper extends ServletRequestWrapper {
        ...
        RequestWrapper(ServletRequest req, String query) {
            super(req);
            mQuery = query;
        }
    ...
    private class ForwardedRequestWrapper extends RequestWrapper {
        ...
        ForwardedRequestWrapper(ServletRequest req,
    ...
    private class IncludedRequestWrapper extends RequestWrapper {
        ...
        IncludedRequestWrapper(ServletRequest req,
                               String uri, String contextPath,
                               String servletPath, String pathInfo,
                               String query) {
            super(req, query);
     ...
     * be ignored.
     */
    private class IncludedResponseWrapper extends ServletResponseWrapper {
        public IncludedResponseWrapper(ServletResponse response) {
            super(response);
        }
